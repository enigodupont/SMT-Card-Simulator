...
pipeline {
  ...
  stages {
    stage('SonarQube Analysis') {
      steps {
        withSonarQubeEnv('Eniworks') {
          container('sonarscan') {
              sh "sonar-scanner -Dsonar.projectKey=SMTONLINE -Dsonar.sources=SMTONLINE"
          }
        }
      }
    }
    stage('Build/Push Docker Image') {
      steps {
        container('kaniko') {
            sh "/kaniko/executor --context . --cache --destination=enigodupont/smtonline:default_tag_change_me --destination=enigodupont/smtonline:$BUILD_NUMBER"
        }
      }
    }
        ...
        withCredentials(bindings: [sshUserPrivateKey(credentialsId: '8fe21223-8d56-49ea-8eb5-6dc0171c102f', keyFileVariable: 'GIT_SSH_KEY', passphraseVariable: '', usernameVariable: '')]) {
          container('git') {
            ...
            sh '''
              ...
              git clone git@github.com:enigodupont/SMT-Card-Simulator.git
              cd SMT-Card-Simulator
